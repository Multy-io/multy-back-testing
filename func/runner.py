        ...
        if case_name in cases:  # and is callable
            try:
                ...
                test_session.logger.error(
                    f'assertion failed in [{case_name}] case with message [{traceback.format_exc()}]')
            except BaseException:
                test_session.inc_assertions_failed()
                test_session.logger.fatal(
                    f'unexpected error in [{case_name}] case with message [{traceback.format_exc()}]')
